...
export class OperationsService {
  ...
  private generateQrCode(ticketCode: string) {
  ...
  async createEntry(body: EntryBody) {
  ...
  async calculateExit(body: ExitCalculateBody) {
    if (!body.ticketCode) {
      throw new BadRequestException({
        success: false,
        error: {
  ...
  async confirmExit(body: ExitConfirmBody) {
    ...
    const lprMismatch =
      !!body.lpr?.plate && !!ticket.vehicle?.plate && body.lpr.plate !== ticket.vehicle.plate;
